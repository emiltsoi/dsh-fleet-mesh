// dsh-fleet-mesh — outbound: build the envelope, sign it, POST it, retry within one budget.
//
...
export function buildSignedRequest({
	...
	// `wireBody` IS the canonical wrap (see envelope.mjs) — it is not a second layer. `raw` sends
	// the bare envelope for a peer that cannot parse the wrap; both forms are accepted inbound.
	//
	// Wrapping and signing are ONE act: the receiver verifies over the bytes it actually
	// received, so whatever is signed here must be exactly what is POSTed.
	const raw = Buffer.from(wireFormat === 'raw' ? envelope : wireBody(from, envelope), 'utf8');
