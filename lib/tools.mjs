...
export function createToolDescriptors(settings, logger = () => {}) {
	return [
		// ---------------------------------------------------------------- list ----
		{
			name: 'mesh_list',
			description:
				'List the mesh peers this agent can reach. Reads the local identity store — which is the VERIFICATION path: peers verify you from their own copy of that store, so a registry row alone makes you discoverable but not verifiable, and their answer to an unverifiable sender is 401. Needs no registry and never touches the network. Read-only.',
			parameters: {
				reachableOnly: {
					type: 'boolean',
		...
		// ---------------------------------------------------------------- send ----
		{
			name: 'mesh_send',
			description:
				'THE LIVE CHANNEL — this starts a turn in the recipient\'s session. It is a prompt, not a notification, and it costs her a turn, so spend it on purpose. Durable mail is NOT this: write a file to your own vault outbox and mesh a short pointer to it instead. "delivered" means her plugin handed it to a session; it does NOT mean she read it — no read receipt exists anywhere in this system. For anything that matters, do both: file the artifact, then send the knock. The envelope is Ed25519-signed and POSTed to her receive endpoint; a failure is reported, never queued silently.',
			parameters: {
		...
		// ---------------------------------------------------------------- sync ----
		{
			name: 'mesh_sync',
			description:
				'Fetch peer identities from the mesh registry and cache them locally, so this deployment can VERIFY those peers. With no agent, syncs every registered peer. Writes only to this deployment\'s own cache directory, never to a shared store — syncing does not make you visible to anyone else, only them visible to you.',
			parameters: {
		...
		// ------------------------------------------------------------ register ----
		{
			name: 'mesh_register',
			description:
				'Publish this agent\'s identity (name, receive URL and Ed25519 public key) to the mesh registry. The registry is DISCOVERY ONLY — it is not on the verification path, so registering here does not by itself let you send or be trusted: peers verify you against their local identity store, and a receiver holding no key for you can only answer 401. Register here AND ensure the row exists in the fleet identity store. The request is signed with this agent\'s own private key.',
			parameters: {
