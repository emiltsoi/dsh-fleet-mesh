...
//   route. So dsh-mesh owns its listener and exposes only /mesh/*.
//
...
function log(ctx, level, message) {
	try {
		...
		if (typeof logger === 'function') logger.call(ctx.logger, `[dsh-mesh] ${message}`);
		else console[level === 'warn' ? 'warn' : 'log'](`[dsh-mesh] ${message}`);
	} catch {
