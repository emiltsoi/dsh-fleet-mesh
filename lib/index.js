...
import { createToolDescriptors } from './tools.mjs';
...
// No injected service is required: the agents service is read through ctx.get, so a
// harness that has not mounted it degrades to an informative 503 instead of failing
// to load this row.
export const inject = [];
...
export function createMeshHandler(ctx, settings) {
	...
	let createUserMessage = (message) => message;
	...
	async function messageHelper() {
		...
		try {
			...
			log(ctx, 'warn', `@deepseek-ai/dsh-llm unavailable, using an identity fallback: ${error?.message ?? error}`);
		}
	...
	async function receive(req, res) {
		...
		const agents = ctx.get?.('agents') ?? ctx.agents ?? null;
...
export function apply(ctx, config = {}) {
	...
	// The tool surface.
	//
	// Two deliberate softnesses. The core import is DYNAMIC because this module is imported
	// by the test suites, which have no @deepseek-ai/* on their resolution path — a static
	// import would make the plugin untestable. And the service is fetched with ctx.get rather
	// than a hard `inject`, so a harness without the tools service still loads the RECEIVE
	// route. Degrade; never break the door.
	ctx.effect(() => {
		const tools = ctx.get?.('tools') ?? ctx.tools ?? null;
		if (!tools || typeof tools.register !== 'function') {
			log(ctx, 'info', 'the tools service is unavailable — mesh_* tools are not registered');
			return () => {};
		}
		const descriptors = createToolDescriptors(settings, (level, message) => log(ctx, level, message));
		let disposers = [];
		let cancelled = false;
		import('@deepseek-ai/dsh-tools')
			.then(({ defineTool }) => {
				if (cancelled) return;
				for (const descriptor of descriptors) disposers.push(tools.register(defineTool(descriptor)));
				log(ctx, 'info', `registered ${disposers.length} mesh tool(s): ${descriptors.map((d) => d.name).join(', ')}`);
			})
