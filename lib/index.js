...
import { unwrapJsonWireBody } from './pyjson.mjs';
...
export function normalizeConfig(config = {}) {
	...
	return {
		...
		// Which wire body this deployment SENDS. `json` is the fleet's canonical default
		// (session_relay.py `wire_format="json"`), wrapping the envelope as `{"from":…,"text":…}`.
		// `raw` is the legacy shape, kept as an escape hatch and for talking to an older peer
		// that cannot parse the wrap. RECEIVING is unaffected: both forms are always accepted,
		// so this can be flipped without stranding anyone mid-conversation.
		wireFormat: config?.wireFormat === 'raw' ? 'raw' : 'json',
...
export function createMeshHandler(ctx, settings) {
	...
	async function receive(req, res) {
		...
		// TWO INBOUND FORMS, ONE VERIFICATION. The canonical wire body is the fleet's json wrap
		// (`{"from": …, "text": …}`); a bare envelope is the legacy `raw` form — which this plugin
		// itself emitted until 0.2.5, and which the fleet still accepts. Both are parsed here, and
		// BOTH are verified against the raw received bytes below. The form decides what to PARSE,
		// never what to trust.
		const asText = raw.toString('utf8');
		const wire = unwrapJsonWireBody(asText);
		const envelope = parseEnvelope(wire ? wire.text : asText);
		// A 400 that logs nothing is undiagnosable — in a test and, worse, in production. Each
		// refusal names what was wrong and, where it is known, who sent it.
		if (!envelope) {
			log(ctx, 'warn', `bad request: no mesh envelope in ${wire ? 'the wrapped text' : 'the raw body'} (${raw.length} bytes)`);
			return sendJson(res, 400, { status: 'bad request' });
		}
		const problems = envelopeProblems(envelope);
		if (problems.length > 0) {
			log(ctx, 'warn', `bad request from "${envelope.from}": ${problems.join(', ')}`);
			return sendJson(res, 400, { status: 'bad request', fields: problems });
		}
		...
		// Only the wrapped form carries a separate `from` to compare.
		if (wire && wire.from !== envelope.from) return sendJson(res, 401, { status: 'unauthorized' });
