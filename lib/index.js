 ...
 * resolve it from the profile tree.
 */
async function loadDefineTool() {
	let url = null;
	try {
		url = import.meta.resolve('@deepseek-ai/dsh-tools');
	} catch {
		url = null;
	}
	if (url === null) {
		...
		if (home) {
			try {
				// Anchoring the require inside the package's own directory makes the walk reach
				// `profiles\node_modules`, where the core is hoisted.
				const anchor = join(home, 'profiles', 'node_modules', '@deepseek-ai', 'dsh-tools', 'package.json');
				url = pathToFileURL(createRequire(anchor).resolve('@deepseek-ai/dsh-tools')).href;
			} catch {
				url = null;
			}
		}
